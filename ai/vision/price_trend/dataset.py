...
class ImagingPriceTrendDataset(Dataset):
    def __init__(self, db_path, img_caching_path, stock_list_file, hist_data_file, seq_length, features, ts_features, scaler, encoder, image_size, min_image_size, tag, is_train=True):
        ...
        self.transforms = transforms.Compose([
            transforms.Resize(max(self.min_image_size, self.image_size), interpolation=transforms.InterpolationMode.NEAREST),
            transforms.ToTensor()
        ])
    ...
    def parse_item(self, idx):
        ...
        return image, _trend, np.clip(acu_return, -3, 3), code, industry, ts_seq, ctx_seq
