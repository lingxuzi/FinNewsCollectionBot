from ai.embedding.models.layers import ALSTMEncoder, ResidualRNNEncoder, LayerNormedResidualMLP, ResidualMLPBlock, nn, torch, F

class TSEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.ts_model = ResidualRNNEncoder(config['ts_input_dim'], config['hidden_dim'], num_layers=config['num_layers'], embedding_dim=config['ts_embedding_dim'], gru=True, kl=False, dropout=0.1)
        self.ctx_model = ResidualMLPBlock(config['ctx_input_dim'], config['hidden_dim'], config['ctx_embedding_dim'],dropout_rate=0, use_batchnorm=True)
        # self.embedding_projector = nn.Linear(config['ts_embedding_dim'] + config['ctx_embedding_dim'], config['hidden_dim'])
        # self.embedding_norm = nn.LayerNorm(config['hidden_dim'])

        full_dim = config['ts_embedding_dim'] + config['ctx_embedding_dim']
        self.fusion_block = ResidualMLPBlock(full_dim, full_dim // 2, config['embedding_dim'], use_batchnorm=False, dropout_rate=config['dropout'])

    def init(self):
        for name, param in self.ts_model.lstm.named_parameters():
            if 'weight' in name:
                nn.init.kaiming_uniform_(param)
            elif 'bias' in name:
                nn.init.zeros_(param)

    def forward(self, x):
        ts_seq, ctx_seq = x
        ts_emb, _, _ = self.ts_model(ts_seq)
        ctx_emb = self.ctx_model(ctx_seq)
        emb = torch.cat([ts_emb, ctx_emb], dim=1)
        # emb = self.embedding_projector(emb)
        # emb = self.embedding_norm(emb)
        emb = self.fusion_block(emb)
        return emb
