...
class TSEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.ts_model = ALSTMEncoder(config['ts_input_dim'], config['hidden_dim'], num_layers=config['num_layers'], embedding_dim=config['ts_embedding_dim'], gru=True, kl=False, dropout=0.1, embedding=True)
        ...
        self.fusion_block = ResidualMLPBlock(config['hidden_dim'], config['hidden_dim'] // 2, config['embedding_dim'], use_batchnorm=False, dropout_rate=config['dropout'])
