...
class ResidualBlockV2(nn.Module):
    def __init__(self, inp, oup, ratio=2, kernel_size=5, stride=1, attention=True, attention_mode='ca'):
        super(ResidualBlockV2, self).__init__()
        self.attention = attention
        
        hidden_dim = round(inp // ratio)

        self.conv = nn.Sequential(
            # dw
            nn.Conv2d(inp, inp, kernel_size, 1, kernel_size//2, groups=inp, bias=False),
            nn.BatchNorm2d(inp),
            nn.SiLU(inplace=True),
            get_attention_module(inp, attention_mode) if attention else nn.Identity(),
            # pw-linear
            ...
            nn.SiLU(inplace=True),
            # dw-linear
            nn.Conv2d(oup, oup, kernel_size, stride, kernel_size//2, groups=oup, bias=False),
            nn.BatchNorm2d(oup),
        )
...
class StockChartNet(nn.Module):
    ...
    def build_conv_groups(self, in_chans, groups=4, channels=[16, 32, 64, 128, 256], kernel_size = 5, attention_mode='ca'):
        if groups == 1:
            ...
            block4 = ResidualBlockV2(channels[2], channels[3], kernel_size=3, stride=2, attention_mode=attention_mode, attention=True)
            block5 = ResidualBlockV2(channels[3], channels[4], kernel_size=3, stride=1, attention_mode=attention_mode, attention=True)
