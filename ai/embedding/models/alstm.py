...
class ALSTMAutoencoder(nn.Module):
    def __init__(self, config):
        ...
        self.predictor = PredictionHead(hidden_dim, predict_dim, act=nn.GELU, dropout_rate=dropout_rate) #ResidualMLPBlock(self.total_embedding_dim, int(hidden_dim), predict_dim, dropout_rate=dropout_rate)
        self.return_head = PredictionHead(hidden_dim, predict_dim, act=nn.GELU, dropout_rate=dropout_rate) #ResidualMLPBlock(self.total_embedding_dim, int(hidden_dim), predict_dim, dropout_rate=dropout_rate)
        self.trend_head = nn.Sequential(
            PredictionHead(hidden_dim, predict_dim, act=nn.GELU, dropout_rate=dropout_rate),
            nn.Sigmoid()
