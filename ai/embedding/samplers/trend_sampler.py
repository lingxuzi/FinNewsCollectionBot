...
from ai.embedding.dataset.dataset import KlineDataset
from tqdm import tqdm
import os
import random

class TrendSampler(torch.utils.data.sampler.Sampler):
    def __init__(self, dataset: KlineDataset, batch_size):
        os.makedirs('../trend_sampler_cache', exist_ok=True)
        self._cache = Cache('../trend_sampler_cache')
        ...
        if len(cls_indices) == 0:
            for i in tqdm(range(len(dataset)), desc='Trend Sampler'):
                ts_seq, ctx_seq, label, trend, _return, date_range, code = dataset.parse_item(i)
